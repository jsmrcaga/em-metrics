...
function run() {
	...
	*/
	...
	if(!process.env.INPUT_CREATE_IF_NOT_EXISTS || !process.env.INPUT_DEPLOYMENT_START_TIME) {
		log.error('create_if_not_exists and deployment_start_time are mandatory when type is DEPLOYED');
		Process.exit(1);
		return Promise.resolve();
	}

	return em_api_client.deployments.deployed({
		create_if_not_exists: Boolean(process.env.INPUT_CREATE_IF_NOT_EXISTS),
		id: process.env.INPUT_DEPLOYMENT_ID,
		project_id: process.env.INPUT_PROJECT_ID,
		first_commit_at: process.env.INPUT_FIRST_COMMIT_AT,
